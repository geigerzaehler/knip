...
export function createCustomModuleResolver(
  ...
  function resolveModuleName(name: string, containingFile: string): ts.ResolvedModuleFull | undefined {
    ...
    if (resolvedFileName) {
      ...
      if (!customCompilerExtensions.includes(ext)) {
    ...
    if (pathMappedFileName) {
      return {
        ...
        extension: getExtension(pathMappedFileName),
        isExternalLibraryImport: false,
      };
    }
  ...
  function getExtension(path: string): string {
    return customCompilerExtensions.some(ext => path.endsWith(ext)) ? ts.Extension.Js : extname(path);
  }
